...
import { useUpdateFoodItem } from "../api/use-update-food-item";
import {
...
interface Props {
  initialData: FoodItemSchemaType;
  foodItemId: string;
  categories: Category[];
}

export function UpdateFoodItemForm({
  initialData,
  foodItemId,
  categories,
}: Props) {
  ...
  const { mutate: updateFoodItem, isPending } = useUpdateFoodItem(foodItemId);
